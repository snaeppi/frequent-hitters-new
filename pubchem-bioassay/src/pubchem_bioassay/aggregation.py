...
from .polars_helpers import is_numeric_dtype
...
def _categorize_columns(schema: dict[str, pl.DataType]) -> tuple[list[str], list[str]]:
    ...
    for name, dtype in schema.items():
        if name == GROUP_KEY:
            continue
        if name in EXCLUDE_GROUP_COLUMNS:
            continue
...
def aggregate_compounds(
    *,
    ...
    lf = pl.scan_parquet(str(input_parquet))
    ...
    aggregations.extend(
        pl.col(col).mode().alias(f"__mode_{col}") for col in categorical_cols
    )
    ...
    if mode_aliases:
        aggregated = (
            aggregated.with_columns(
                [
                    pl.col(alias).list.first().alias(col)
                    for col, alias in mode_aliases.items()
                ]
            ).drop(list(mode_aliases.values()))
        )
    aggregated.sink_parquet(str(output_parquet))
