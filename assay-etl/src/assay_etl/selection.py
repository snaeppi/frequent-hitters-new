...
def interactive_select_for_aid(
    *,
    ...
    Expects the aggregated parquet generated by download-assays (aid_<AID>.parquet).
    """
    if not aggregated_parquet.exists():
        raise FileNotFoundError(
            f"Aggregated parquet not found for AID {aid}: {aggregated_parquet} "
            "(run download-assays first)."
        )

    row_count, candidates = _compute_candidate_stats(aggregated_parquet=aggregated_parquet)
    if not candidates:
